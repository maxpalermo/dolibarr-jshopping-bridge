...
if($result)
{
    while($rs = $db->fetch_object($result))
    {
        ...
        $valueFields = substr($valueFields,0,-2) . "\n);";
        ...
        ***********************************************************************/
        $exportSQL = $sqlFields . $valueFields;
        $resProd = $dbOut->query($exportSQL);
        if(!$resProd)
        {
            print "EXP PROD (err " . $db->lasterrno() . ") " . $db->lasterror() . ", id: $rs->rowid \n"; // . "\n" . $exportSQL . "\n";
        }
