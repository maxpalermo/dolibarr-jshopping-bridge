...
//GET DOLIBARR CATEGORIES ID
$sqlCatIdDolibarr = "select rowid from " . MAIN_DB_PREFIX . "categorie order by rowid";
...
$sqlCatIdJShopping = "select category_id from " . mysqlParams::getPrefix() . "jshopping_categories order by category_id";
...
else
{
    ?>
        <div>
            <h3>ERRORE</h3>
            <p><i>Errore durante la lettura delle categorie di <strong>JShopping</strong></i></p>
        </div>
    <?php
    return;
}
    ...
    ?>
        <tr>
            <td colspan="3"> 
                <div>
                    <h3>ATTENZIONE</h3>
                    <p><i>Nessuna categoria nuova da esportare</i></p>
                </div>
            </td>
        </tr>
    <?php
...
{
    $sqlCategories = "select * from " . MAIN_DB_PREFIX . "categorie where rowid in (" . implode(",",$diff) . ") order by rowid;";
